module type S = module type of Buf
...
let check b0 b1 =
  ...
  if !print then
    ...
    let print (i,p) = Printf.sprintf "[%d]:%d" i p in
    Printf.eprintf "B0: %s\nB1: %s\n"
      (String.concat " " (List.map print b0))
      (String.concat " " (List.map print b1))
