...
module Class = struct
  ...
  and buffer = Buffer : 'a t * 'a -> buffer

  let make c i = Buffer (c, i)
end

type buffer = Class.buffer

let connect ~a ~b =
  let Class.Buffer (ca, ia) = a in
  let Class.Buffer (cb, ib) = b in
  ca.Class.connect ia b;
  cb.Class.connect ib a;
  ca.Class.connected ia;
  cb.Class.connected ib

let click (Class.Buffer (c, i)) offset =
  c.Class.click i offset

let change (Class.Buffer (c, i))
...
module Prim = struct
  ...
  let create () =
    ...
    let buffer = { commands; buffer = null;
  ...
  let class_ = {
    Class.
    connect = (fun t buf -> t.buffer <- buf);
    connected = ignore; change; click;
  }
end

let session () =
  let session, t = Prim.create () in
  session, Class.Buffer (Prim.class_, t)
